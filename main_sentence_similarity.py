...
train_texts, _ = utils.load_data_label(train_filepath)
...
trained_model_checkpoint_dir_pvdbow = 'model/trained_model_pvdbow_300d/checkpoints'
trained_model_checkpoint_dir_pvdm = 'model/trained_model_pvdm_300d/checkpoints'
...
sim_matric_argmax_ix = k_most_common(test_embeddings, doc_id)  # 计算相似度，LabeledSentences_train前十个句子
logging.info('-' * 80)
logging.info(
    'target sentence: \n'
    + '\t\t' + train_texts[doc_id] + '\n'
    + '\t\t' + ' '.join(utils.ids2words(index2word, utils.words2ids(word2index, train_texts[doc_id].split(
        ' ')))))

for sim_t in sim_matric_argmax_ix:
    sim_t = int(sim_t)
    logging.info('-' * 100 + '\n'
                 + '\t\t' + train_texts[sim_t] + '\n'
