...
from _textrnn.textrnn_model import TextRNN
...
tf.app.flags.DEFINE_integer("validate_every", 5, "Validate every validate_every epochs.")  # 每x轮做一次验证
...
tf.app.flags.DEFINE_integer("hidden_size", 300, "lstm cell hidden layer size")
...
def main(_):
    ...
    if os.path.exists(FLAGS.cache_path):
        ...
        if use_pretrained_word_embeddings:
            ...
            word2index={}# word2index = {'_UNK_': 0}
            word2vec_dict['_UNK_']= np.zeros(FLAGS.embed_size) # word2vec_dict['_NULL_'] = np.zeros(FLAGS.embed_size)
    ...
    class_weights = tf.constant([1.0, 0.1])
    train(trainX, trainY, devX, devY, testX, testY, word2vec_dict, index2word, vocab_size, FLAGS.embed_size,
          label_index, class_weights)


def train(trainX, trainY, devX, devY, testX, testY, word2vec_dict, index2word, vocab_size, embed_size, label_index,
          class_weights):
    ...
    with tf.Session(config=config) as sess:
        ...
        textRNN = TextRNN(FLAGS.num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps, FLAGS.decay_rate,
                          ...
                          vocab_size, FLAGS.embed_size, FLAGS.hidden_size,FLAGS.is_training, class_weights)  # Instantiate Model
        ...
        if os.path.exists(FLAGS.ckpt_dir + "checkpoint"):  # for continue training
            ...
            if FLAGS.use_embedding:  # assign pre-trained word embedding
                assign_pretrained_word_embedding(sess, word2vec_dict, index2word, embed_size, textRNN)
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs + 1):  # 3.feed data & training
            ...
            for batch_num in range(num_batches_per_epoch):
                ...
                curr_loss, curr_acc, _ = sess.run([textRNN.loss_val, textRNN.accuracy, textRNN.train_op],
                                                  feed_dict={textRNN.input_x: trainX[start_index:end_index],
                                                             textRNN.input_y: trainY[start_index:end_index],
            ...
            if epoch % FLAGS.validate_every == 0 or epoch == FLAGS.num_epochs or tol == 0:  # 4.validation
                ...
                do_eval_confusion_matrix(sess, textRNN, devX, devY, 2, label_index)
                save_path = FLAGS.ckpt_dir + "model.ckpt"
                saver.save(sess, save_path, global_step=epoch)  # save model to checkpoint
        ...
        do_eval_confusion_matrix(sess, textRNN, testX, testY, 2, label_index)
